...
namespace SecurityDAL
{
	public class s_RoleDAO //: IDisposible
	{
		...
		public List<s_Role> GetAll(Int32? RoleId = null)
		{
			try
			{
		...
		public List<s_Role> GetDynamic(string whereCondition,string orderByExpression)
		{
			try
			{
				...
				s_RoleLst = dbExecutor.FetchData<s_Role>(CommandType.StoredProcedure, "s_Role_GetDynamic", colparameters);
				return s_RoleLst;
			}
			catch (Exception ex)
			{
		...
		public int Add(s_Role _s_Role)
		{
			...
			try
			{
				...
				new Parameters("@IsCheckoutOperator", _s_Role.IsCheckoutOperator, DbType.Boolean, ParameterDirection.Input),
				...
				ret = dbExecutor.ExecuteScalar32(true, CommandType.StoredProcedure, "s_Role_Create", colparameters, true);
