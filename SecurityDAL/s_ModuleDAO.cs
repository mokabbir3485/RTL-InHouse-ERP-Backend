	...
	public class s_ModuleDAO //: IDisposible
	{
		...
		public List<s_Module> GetByDomainId(Int32? domainId)
		{
			try
			{
                ...
                s_ModuleLst = dbExecutor.FetchData<s_Module>(CommandType.StoredProcedure, "s_Module_GetByDomainId", colparameters);
				return s_ModuleLst;
			}
			catch (Exception ex)
			{
        ...
        public List<s_Module> GetExAdminSecurity()
        {
            ...
            {
                List<s_Module> s_ModuleLst = new List<s_Module>();
                s_ModuleLst = dbExecutor.FetchData<s_Module>(CommandType.StoredProcedure, "s_Module_GetExAdminSecurity");
                return s_ModuleLst;
            }
            catch (Exception ex)
            {
		...
		public int Add(s_Module s_Module)
		{
			...
			try
			{
				...
				ret = dbExecutor.ExecuteScalar32(true, CommandType.StoredProcedure, "s_Module_Create", colparameters, true);
		...
		public int Delete(Int32 moduleId)
		{
			try
			{
				int ret = 0;
				...
				ret = dbExecutor.ExecuteNonQuery(CommandType.StoredProcedure, "s_Module_DeleteById", colparameters, true);
				return ret;
			}
			catch (Exception ex)
			{
